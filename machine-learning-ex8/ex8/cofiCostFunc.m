...
%Vectorized implemenetation to compute grad
%The usuall way using for loop has much computational cost(about hours)

%To learn X
for i = 1 : num_movies
    idx = find(R(i, :) == 1);   %idx is a list of all the users that have rated movie i
...
%To learn Theta
for j = 1 : num_users
    idx = find(R(:, j) == 1);   %idx is a list of all the movies that user j have rated
    X_temp = X(idx, :);
    Y_temp = Y(idx, j);
